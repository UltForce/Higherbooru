...
<div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="search-function">
        <form method="GET" action="{{ route('posts.index') }}" class="flex items-center mb-4">
            <!-- Search Input -->
            ...
            <!-- Clear Search Button -->
            <a href="{{ route('posts.index') }}" class="clear-btn text-blue-500 hover:text-blue-700 ml-4 clear-btn">Clear Search</a>
        </form>
    </div>
        ...
        @foreach ($posts as $post)
                    ...
                    <div class="p-4">
                        <h2 class="image-title text-lg font-bold text-gray-800">{{ $post->title }}</h2>
                        <p class="image-uploader text-gray-500 text-sm">Uploaded by: {{ $post->user->name }}</p>
                        <div class="mt-2">
                            <strong>Tags:</strong>
                            <ul class="image-tags-text list-none">
                                @foreach ($post->tags as $tag)
                                    <li class="image-tags inline-block text-sm text-white">{{ $tag->name }}</li>
                                @endforeach
                            </ul>
                        </div>
                        <!-- Bookmark Button -->
                        ...
                        <form action="{{ route('posts.bookmark', $post->id) }}" method="POST">
                            ...
                            <span class="image-bookmark-cntr text-sm text-gray-500">{{ $post->bookmarkCount() }} bookmarks</span>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="pagination-btn mt-6">
            {{ $posts->appends(request()->query())->links() }} <!-- Ensure query parameters are kept in pagination links -->
        </div>
    </div>
    </body>
    <!-- SweetAlert Script -->
